...
def make_chart(json_url, songs_dir, players_dir, border):
    ...
    pname = []
    got_name = []
    frag = not(songs_dir == "songs_satellite" or songs_dir == "songs_stella" or songs_dir == "songs_insane" or songs_dir == "songs_dpsatellite")
    print(frag)
    ...
    for song in json_data:
            ...
            for IR_player in IR_list:
                ...
                if(player_ID in black_list):
                    ...
                    if(score_rate >= border):
                        ...
                        if(frag):
                            try:
                                lr2_score[player_ID].append([s_title, s_md5, player_score, user_lank, min_BP])
                            except KeyError:
                                lr2_score[player_ID] = [[songs_title[i], songs_md5[i], 0, 0, 0] for i in range(len(songs_md5))]
                                lr2_score[player_ID].append([s_title, s_md5, player_score, user_lank, min_BP])
        ...
        if players < 12: #プレイヤー人数で場合分け
            ...
            great_p = 1.8
            min_bunsan = 1e9
            for kouho_p in range(1, 181):#p=[0.01, 1.5]で最小二乗法
                BPI_zissai = []
                kouho_p /= 100
                for bs in BPI_score:
                    if(bs <= max_score):
        ...
        if(song['level'] == '？' or song['level'] == '???' or song['level'] == '?'):
            song['level'] = 99      
        else:
    ...
    if(frag):
        for user in pname:
            lr2_score[user[0]].append(user[1])

    with open("bpitool/score_data/" + songs_dir + ".json", 'w') as f:
        json.dump(songs_data, f, ensure_ascii=False, indent=4)
    
    if(frag):
        with open("bpitool/score_data/" + players_dir + ".json", "w") as f:
            json.dump(lr2_score, f, ensure_ascii=False)